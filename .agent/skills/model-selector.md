...
> [!NOTE]
> **CLI Worker (`GEMINI-CLI/Worker`):** Not a model switch — this delegates the task to a Gemini CLI process via the `worker-delegate` skill and `/delegate-task` workflow. Use when the task qualifies per the Decision Gate (scoped, independent, context-heavy). See `agent-behavior.md §6.1`.
